...
baseDir = os.getcwd()
os.chdir(os.path.join(baseDir,'yahoodata'))
...
class UpdateData():
    ...
    def UpdateTransactions(self):
        ...
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/380.l.712745/transactions'
    ...
    def UpdateLeague(self):
        # LEAGUE OVERVIEW
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/380.l.712745/'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateLeagueStandings(self):
        # STANDINGS
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/380.l.712745/standings'
        response = oauth.session.get(url, params={'format': 'json'})
        print(type(response))
        r = response.json()
        print(type(r))
        standingsJSON = json.dumps(r)
    ...
    def UpdateScoreboards(self):
        # WEEKLY SCORE BOARD
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/380.l.712745/scoreboard;week='+str(current_week)
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
        scoreboardJSON = json.dumps(r)
    ...
    def UpdateRosters(self):
        ...
        for week in range(1, num_weeks+1): #assumes 16-week schedule
            ...
            for team in range(1, num_teams+1): #assumes 12-team league
                url = 'https://fantasysports.yahooapis.com/fantasy/v2/team/380.l.712745.t.'+str(team)+'/roster;week='+str(week)
                response = oauth.session.get(url, params={'format': 'json'})
                r = response.json()
...
class Bot():
    ...
    def getStandings(self):
        UD = UpdateData()
        standingsJSON = UD.UpdateLeagueStandings()
        return standingsJSON;

    def getScores(self):
        UD = UpdateData()
        scoreboardJSON = UD.UpdateScoreboards()
        return scoreboardJSON;
...
#### Where the tweets happen ####
    bot = Bot(yahoo_api)
    standingsJSON = bot.getStandings()
    scoreboardJSON = bot.getScores()
    print(standingsJSON)
